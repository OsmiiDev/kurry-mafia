...
@inject(delay(() => Store)) private store: Store,
    ...
    async initialize() {
        const pluginsManager = await resolveDependency<PluginsManager>(PluginsManager)

        // get config
        const config = mikroORMConfig.production as Options<BetterSqliteDriver>

        // defines entities into the config
        config.entities = [...Object.values(entities), ...pluginsManager.getEntities()] as any
    ...
    get orm(): MikroORM<BetterSqliteDriver> {
        return this._orm
    }
    ...
    isSQLiteDatabase(): boolean {
        return mikroORMConfig[env.NODE_ENV]!.driver === BetterSqliteDriver
    }
