...
export async function syncUser(user: DUser) {
    ...
    if (!userData) {
        ...
        newUser.id = user.id
        await userRepo.getEntityManager().persistAndFlush(newUser)
...
export async function syncGuild(guildId: string, client: Client) {
    ...
    if (!guildData) {
        ...
        if (deletedGuildData) {
            ...
            await guildRepo.getEntityManager().persistAndFlush(deletedGuildData)
            ...
            newGuild.id = guildId
            await guildRepo.getEntityManager().persistAndFlush(newGuild)
        ...
        await guildRepo.getEntityManager().persistAndFlush(guildData)
