...
import { apiConfig, dashboardConfig, generalConfig } from '@/configs'
import { keptInstances } from '@/decorators'
import { checkEnvironmentVariables, env } from '@/env'
import { NoBotTokenError } from '@/errors'
import { Database, ErrorHandler, EventManager, HealthMonitor, ImagesUpload, Logger, PluginsManager, Store } from '@/services'
...
async function init() {
    ...
    const configJson = JSON.parse(readFileSync('./assets/config.json', 'utf-8'))
    Object.assign(dashboardConfig, configJson)
        ...
        client.login(env.BOT_TOKEN)
            .then(async () => {
                ...
                if (apiConfig.enabled) {
                    ...
                    const monSvc = await resolveDependency(HealthMonitor)
                    await monSvc.initialize()
                }
